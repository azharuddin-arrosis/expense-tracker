...
export function CategoryBar({ data, total, showPercentage = true, categoryBudgets }: CategoryBarProps) {
  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
      <div className="grid grid-cols-[1fr_1fr] gap-0 border-b border-gray-200 bg-gray-50 text-[10px] font-semibold text-gray-500 uppercase tracking-wider">
        <div className="px-2.5 py-1.5 border-r border-gray-200">Kategori</div>
        <div className="px-2.5 py-1.5 text-right">Nominal</div>
      </div>
      {items.map((item, i) => (
        <div key={item.categoryId} className={`grid grid-cols-[1fr_1fr] gap-0 text-xs border-b border-gray-100 ${i % 2 === 0 ? 'bg-white' : 'bg-gray-50/50'}`}>
          ...
          <div className="px-2.5 py-2 text-right">
            <span className="font-semibold text-gray-900 tabular-nums">{formatRupiah(item.total)}</span>
            <span className="text-gray-400 tabular-nums ml-1">({item.percentage.toFixed(1)}%)</span>
          </div>
        </div>
      ))}
      <div className="grid grid-cols-[1fr_1fr] gap-0 border-t border-gray-200 bg-gray-50 text-xs font-bold text-gray-900">
        <div className="px-2.5 py-1.5 border-r border-gray-200 text-gray-500 font-medium">Total</div>
        <div className="px-2.5 py-1.5 text-right tabular-nums">{formatRupiah(total)} — 100%</div>
      </div>
    </div>
  );
}
